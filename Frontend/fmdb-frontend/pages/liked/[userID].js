...
const UserLikedMoviesPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchLikedMovies = async () => {
        setLoading(true);
        setError(null);
        setIsPrivate(false); // Reset the private flag at the start
        ...
        try {
            ...
            if (!token) {
                ...
                url = `http://localhost:5000/users/likedMovies/public/${userID}`;
            } else {
                ...
                if (user && parseInt(user.userID) === parseInt(userID)) {
                    url = `http://localhost:5000/users/likedMovies/${userID}`; // Allow access to own liked movies
                } else {
                    // Case 2: For other users, use the general endpoint
                    url = `http://localhost:5000/users/likedMovies/${userID}`;
                }
            }
            ...
            if (data.success) {
                setMovies(data.likedMovies || []); // Ensure likedMovies is set
                console.log('Liked Movies received:', data.likedMovies); // Log the received likedMovies
                ...
                if (data.message === 'User profile is private') {
                    ...
                    setMovies([]); // Empty likedMovies on the frontend if user profile is private
                    if (data.username) setProfileUsername(data.username);
                }
            }
        } catch (err) {
            console.error('Error fetching liked movies:', err);
            setError('An error occurred while fetching the liked movies');
        } finally {
            setLoading(false);
        }
    };
  ...
  // Determine if this is the current user's own liked movies
