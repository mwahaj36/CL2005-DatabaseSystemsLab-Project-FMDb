...
const TrendingReviews = ({ reviewsData = [], user, movie = {} }) => {
  ...
  const checkDeletableStatus = async (activityId) => {
    ...
    try {
      const response = await fetch(
        `https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/activity/isDeletable/${activityId}`,
        {
  ...
  useEffect(() => {
    ...
    const fetchStatuses = async () => {
      ...
      for (let review of reviewsData) {
        try {
          const likeUrl = new URL(
            'https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/activity/isLiked'
          );
          ...
          deletableStatusMap[review.ActivityID] = await checkDeletableStatus(
            review.ActivityID
          );
        } catch (err) {
  ...
  const handleDeleteReview = async (activityId) => {
    ...
    try {
      const response = await fetch(
        `https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/activity/${activityId}`,
        {
          method: 'DELETE',
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      setReviews((prev) =>
        prev.filter((review) => review.ActivityID !== activityId)
      );
    } catch (err) {
  ...
  return (
    <div className="px-4">
      <h2 className="text-purpleWhite text-xl md:text-2xl font-bold mb-4 text-center">
        Trending Reviews
      </h2>
        ...
        {reviews.map((review) => {
          ...
          return (
            <div
              ...
              className="flex flex-col items-center bg-black bg-opacity-60 p-3 rounded-2xl shadow-md hover:scale-105 transition-transform duration-300"
            >
              <div
                className={`font-bold text-darkPurple text-lg px-4 py-6 rounded-xl shadow-inner ${
                  review.userType === 'verified critic'
                    ? 'bg-gold'
                    : 'bg-purpleWhite'
                }`}
              >
              ...
              <div className="mt-4 text-center">
                <p className="text-white italic text-sm">{review.Review}</p>
                <p className="text-purpleWhite font-bold mt-2 text-xs">
                  {review.Username || 'Anonymous'}
                </p>
              </div>

              <div className="flex items-center space-x-4 mt-4">
                <span className="text-white text-sm">{review.ActivityLikeCount || 0}</span>
                <button
      ...
      <div className="mt-4 text-center">
        <button
          ...
          className="bg-purple text-white py-2 px-4 rounded-full hover:bg-darkPurple transition-colors duration-200 text-sm"
        >
          See All Reviews
        </button>
      </div>
