...
const MoviesListPage = () => {
  // State for regular movie listing
  ...
  const fetchMovies = async () => {
    ...
    try {
     const response = await fetch(
      `${API_URL}/movies/page?page=${page}&sort=${sort}&order=${order}`,
      {
        headers: {
  ...
  const fetchPageCount = async () => {
   try {
  const response = await fetch(`${API_URL}/movies/pageCount`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      if (params.toString()) {
       const response = await fetch(
        `${API_URL}/movies/discover?${params.toString()}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
              ...
              {showAdvancedSearch && (
                <div className="mt-4 bg-darkPurple bg-opacity-90 p-4 rounded-lg border border-purple-500">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-purple-200 text-sm mb-1">Movie Length (minutes)</label>
                      <input
                        type="number"
                        name="movieLength"
