    ...
    const fetchUserProfile = async () => {
      try {
        ...
        let endpoint = currentUser
          ? `https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/users/logged/${userID}`
          : `https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/users/public/${userID}`;
    ...
    const fetchFriendStatus = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/users/isFriend/${userID}`,
          {
            headers: {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data.message || 'Failed to fetch friend status');
  ...
  const requestUserTypeChange = async type => {
    ...
    try {
      const res = await fetch(
        'https://fmdb-server-fmf2e0g7dqfuh0hx.australiaeast-01.azurewebsites.net/users/userType',
        {
          method: 'PUT',
          headers: {
        ...
        <div className="relative p-4 z-10">
          ...
          {showPrivateContent ? (
            <>
              ...
              <div className="relative flex flex-col z-10 mt-6 space-y-10 md:flex-row md:space-x-6 md:space-y-0">
            ...
            <div className="mt-6 text-center">
              <p className="text-white mt-6 font-semibold text-center text-xl md:text-2xl">
                ...
                {currentUser
                  ...
                  : 'Log in and become friends to see more!'}
              </p>
            </div>
          )}
        </div>
